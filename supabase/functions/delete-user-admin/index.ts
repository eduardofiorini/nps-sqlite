...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Admin ${user.id} deleting user: ${user_id}`);
    ...
    try {
      ...
      if (authDeleteError) {
        ...
        return new Response(
          JSON.stringify({ 
            success: false,
            error: `Failed to remove authentication: ${authDeleteError.message}`,
            message: 'User data deleted but authentication removal failed'
          }),
          {
            status: 500,
            headers: { 'Content-Type': 'application/json', ...corsHeaders },
