...
const { AppConfig, Campaign, Contact } = require('../models');
...
router.post('/campaign', authenticateToken, async (req, res) => {
  try {
    ...
    if (!config || !config.integrations?.smtp?.enabled) {
    ...
    const campaign = await Campaign.findOne({
      where: { id: campaignId, user_id: req.user.id }
    });
...
function personalizeContent(content, contact, campaign, surveyUrl) {
  return content
...
function createEmailTemplate(message, contact, campaign, surveyUrl, includeLink, fromName) {
  return `
